...
function WorkspaceEditor({ projectId, workspace, user }: { projectId: string; workspace: Workspace; user: User }) {
  ...
  const compileError = typeof job.data?.result.error_summary === "string" ? job.data.result.error_summary : typeof job.data?.error_message === "string" && job.data.error_message ? job.data.error_message : "Compile gagal.";
